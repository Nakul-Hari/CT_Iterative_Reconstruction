...
while reconstruction_error > 0.1
    for angles = randperm(num_angles) - 1
        index = angles * detectors + 1 : angles* detectors + detectors;
        calculated_radon = system_matrix(index, 1:num_rows * num_cols) * previous_image;
        error = Radon(index) - calculated_radon;
        length = sum(system_matrix(index,:),2);
